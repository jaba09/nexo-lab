...
test("creates the independent SQLite database with the migrated hierarchy", async () => {
  ...
  assert.equal(database.prepare("SELECT COUNT(*) AS total FROM academic_day_types WHERE day_type = 'A'").get().total, 59);
  assert.equal(database.prepare("SELECT COUNT(*) AS total FROM academic_day_types WHERE day_type = 'B'").get().total, 61);
